...
function ShowItem(data,row,page){
    ...
    for (const author of data.slice(start, end)) {
        $("#bookData").append(
            ...
            <td><a href='../View/editAuthor.php?id= ${author.id}'><button type='button' class='btn btn-outline-primary'><ion-icon name='create-outline'></ion-icon></button></a></td>
            <td><a href='../Controller/deleteAuthorController.php?id= ${author.id}'><button type='button' class='btn btn-outline-danger'><ion-icon name='trash-outline'></ion-icon></button></a></td>
            </tr>`
        )
        }
}
